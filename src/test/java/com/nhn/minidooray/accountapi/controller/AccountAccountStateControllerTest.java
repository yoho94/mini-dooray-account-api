...
class AccountAccountStateControllerTest {
    ...
    void createAccountAccountState_ValidRequest_ReturnsCreated() throws Exception {
        ...
        mockMvc.perform(post("/account-api/account-account-state")
            ...
            .andExpect(jsonPath("$.header.resultMessage").value("AccountAccountState created successfully"));
    }

    @Test
    void createAccountAccountState_InvalidRequest_ThrowsValidationFailedException() throws Exception {
        AccountAccountCreateRequest request = AccountAccountCreateRequest.builder()
            .accountId("invalidId")
            .accountStateCode(AccountStateType.DORMANT.getCode() )
            .build();
    ...
    void getAccountWithStateByAccount_ValidRequest_ReturnsAccountStateList() throws Exception {
        ...
        when(accountAccountStateService.getByAccount(anyString(), any(Pageable.class))).thenReturn(page);
        when(apiMessageProperties.getGetSuccMessage()).thenReturn("success");
        mockMvc.perform(get("/account-api/account-account-state/"+"testId1"+"/list", "testId1")
                .param("page", "0")
                .param("size", "10")
                .param("sort", "accountId,asc"))
            .andExpect(jsonPath("$.header.successful").value(true))
            .andExpect(jsonPath("$.header.resultCode").value(HttpStatus.OK.value()))
            .andExpect(jsonPath("$.header.resultMessage").value("All account-account-states by account ID success"))
            .andExpect(jsonPath("$.result[0].content").isArray())
            .andExpect(jsonPath("$.result[0].content[0].accountId").value(accountList.get(0).getAccountId()))
            .andExpect(jsonPath("$.result[0].content[0].stateCode").value(accountList.get(0).getStateCode()));
    }
